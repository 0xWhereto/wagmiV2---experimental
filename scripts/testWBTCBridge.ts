...
// Amount: 0.00001 WBTC = 1000 satoshis (minimum bridge amount)
const BRIDGE_AMOUNT = ethers.utils.parseUnits("0.00001", 8);

async function main() {
  console.log("=== TESTING WBTC BRIDGE ===\n");
  ...
  if (balance.lt(BRIDGE_AMOUNT)) {
    console.log("❌ Not enough WBTC to test");
    return;
  }
  ...
  console.log(`sWBTC balance on Sonic: ${ethers.utils.formatUnits(sbtcBalanceBefore, 8)}`);
  ...
  console.log(`\nDeposit fee: ${ethers.utils.formatEther(fee)} ETH`);
  ...
  console.log("\nWait 1-2 minutes for message delivery, then check sWBTC balance on Sonic.");
}
