...
contract MIM is ERC20, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    /// @notice Fee tier for MIM/USDC pool (0.1% = 1000, but using 0.05% = 500 for stables)
    uint24 public constant POOL_FEE = 500; // 0.05% fee tier for stablecoins
    
    /// @notice Tick range for 0.995 - 1.005 price range
    /// For USDC/MIM at 1:1, tick 0 = price 1.0
    /// tick -10 ≈ 0.999, tick +10 ≈ 1.001
    /// For tighter 0.995-1.005: tick -50 to +50 (approximately)
    int24 public constant TICK_LOWER = -100; // ~0.99
    int24 public constant TICK_UPPER = 100;  // ~1.01
        ...
        Ownable(msg.sender)
    {
        ...
        // Will approve MIM after deployment (need to approve self)
    }
    
    /**
     * @notice Initialize approvals (call after deployment)
     */
