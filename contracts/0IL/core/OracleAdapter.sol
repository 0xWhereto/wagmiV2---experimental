...
contract OracleAdapter is Ownable {
    ...
    /// @notice Whether to invert the price (return 1/price)
    /// @dev Set to true when underlying asset is token1, false when token0
    ...
    event InvertPriceSet(bool invert);
     ...
     * @dev Set to true when you want price of token1 in terms of token0
     *      (e.g., when underlying asset is token1)
     */
    function setInvertPrice(bool _invert) external onlyOwner {
        invertPrice = _invert;
        emit InvertPriceSet(_invert);
    }
     ...
     * @dev Returns price of token1 in terms of token0 if invertPrice is true,
     *      otherwise returns price of token0 in terms of token1
    ...
    function getPrice() external view returns (uint256 price) {
        ...
        // Invert if needed (for when underlying asset is token1)
