...
contract LeverageAMM is Ownable, ReentrancyGuard {
    ...
    /// @notice Whether underlying asset is token0 in the V3 pool
    ...
    constructor(
        ...
        // In Uniswap V3, token0 < token1 by address
    ...
    function getTotalLPValue() public view returns (uint256) {
        ...
        // Calculate value based on token ordering
        if (underlyingIsToken0) {
            // token0 is underlying, token1 is MIM
            return (amount0 * price / WAD) + amount1;
        } else {
            // token0 is MIM, token1 is underlying
    ...
    function openPosition(uint256 underlyingAmount) external onlyWToken nonReentrant {
        ...
        // V3LPVault expects (token0Amount, token1Amount)
        if (underlyingIsToken0) {
            // underlying is token0, MIM is token1
            ...
            // MIM is token0, underlying is token1
            v3LPVault.addLiquidity(
                borrowAmount,
                underlyingAmount,
                0, // No slippage protection for internal tx
    ...
    function closePosition(
        ...
        // Get underlying and MIM amounts based on token ordering
    ...
    function _deleverage(uint256 repayAmount) internal {
        ...
        uint256 actualRepay = mimAmount > repayAmount ? repayAmount : mimAmount;
        ...
        if (underlyingAmount > 0) {
            if (underlyingIsToken0) {
                v3LPVault.addLiquidity(underlyingAmount, 0, 0, 0);
            } else {
                v3LPVault.addLiquidity(0, underlyingAmount, 0, 0);
            }
        }
    }
    ...
    function _addLeverage(uint256 borrowAmount) internal {
        ...
        // Add the borrowed MIM to LP - respect token ordering
        if (underlyingIsToken0) {
            // MIM is token1
    ...
    function collectAllFees() public nonReentrant returns (uint256 totalFeesInMIM) {
        ...
        // Get underlying and MIM fee amounts based on token ordering
        ...
        // Convert underlying fees to MIM value
        uint256 price = oracle.getPrice();
