...
describe("Synthetic Token System", function () {
  ...
  describe("Cross-Chain Token Flow", function () {
    ...
    it("should successfully lock tokens in gateway vault", async function () {
      ...
      await gatewayVault
        .connect(user1)
        .deposit(user2.address, [{ tokenIndex: 0, tokenAmount: depositAmount }], options, { value: nativeFee });
    ...
    it("should perform full cross-chain token cycle with multi-chain bridging (B->A->C)", async function () {
      ...
      await gatewayVault
        .connect(user1)
        .deposit(user2.address, [{ tokenIndex: 0, tokenAmount: depositAmount }], optionsSendB, {
          value: nativeFeeSendB,
        });
      ...
      await gatewayVaultC
        .connect(user3)
        .deposit(user2.address, [{ tokenIndex: 0, tokenAmount: depositAmountDiffDecimals }], optionsSendC, {
